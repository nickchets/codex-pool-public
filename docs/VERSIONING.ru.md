...
- `0.10.2`: fix для scoped GitLab Claude shared-TPM cooldown, чтобы здоровые sibling seat'ы не выпадали из routable lane
- `0.10.3`: recovery canary для GitLab Claude, persist'ed local Codex stream cooldown и lower-inflight seat reservation для fresh Codex work
- `0.10.4`: hardening export-path и helper-driven cleanup для dashboard assembly, retry bookkeeping, provider boilerplate и persisted health writes
